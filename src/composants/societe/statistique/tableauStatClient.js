...
function createDataAnnée(nomClient,ob) {
   var obj={nomClient}
...
export default function TableauStatClient() {
  const [dataclient,setDataclient]=useState({nomClient:"",labels:[],datasets:[],totale:0})
  ...
  const handleFilters = async (filters, category) => {
    ...
    getStatClient(newFilters);
  }
  

  useEffect(() => {
    getStatClient(Filters);
    getIntervenants();
  }, [])
  ...
  const getStatClient = async(filt)=>{
    ...
    getStat(val._id,val.raisonSociale,filt) 
    })   
  }
  const getStat = async(idclient,nomclient,filter)=>{
    ...
    rows.push(createDataAnnée(nomclient,res.data.data))
    }else{
    rows.push(createDatamoin(nomclient,res.data.data.values[0],res.data.data.values[1],res.data.data.values[2],res.data.data.values[3],res.data.data.values[4],res.data.data.values[5],res.data.data.values[6],res.data.data.values[7],res.data.data.values[8],res.data.data.values[9],res.data.data.values[10],res.data.data.values[11]))
    }
  ...
  const getheadCells =(filter)=>{
    ...
    headCells.push ({ id: 'nomClient', numeric: false, disablePadding: true, label: 'Nom Client' })
    labels[0].map((val)=>{ 
      if(filter["year"] === 0){
